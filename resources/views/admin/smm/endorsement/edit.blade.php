...
<x-main-layout breadcumb="SMM / Direct Job Order" page="Create Direct Job Order">
    ...
    <div class="w-full px-6 py-10 mx-auto rounded-lg custom-shadow bg-white">
        <div class="w-fit">
            <a href="{{ url('/admin/smm/endorsement') }}">
                <div class="w-fit px-4 py-1 bg-gray-400 rounded-md text-white custom-shadow custom-hover-shadow">
                    Back
                </div>
            </a>
        </div>
        <form action="{{ route('admin.smm.endorsement.update', $endorsement->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-4 space-y-4">
                <div class="col-span-4 grid grid-cols-2 gap-4 mt-4">
                    <div class="col-span-2 lg:col-span-1 w-full">
                        ...
                        <input type="text" name="title" value="{{ old('title', $endorsement->title) }}"
                    ...
                    <div class="col-span-2 lg:col-span-1 w-full">
                        ...
                        <div class="relative">
                            @php
                                $selectedClientId = old('client_id', $endorsement->client_id ?? '');
                                $selectedClientName =
                                    $clients->firstWhere('id', $selectedClientId)->name ?? 'Select a Client';
                            @endphp

                            <input type="text" id="selected-client-name" value="{{ $selectedClientName }}"
                    ...
                    <div class="col-span-2 grid grid-cols-2 w-full gap-4 rounded-lg">
                        <div>
                            ...
                            <div class="relative">
                                @php
                                    $selectedUserId = old('person_in_charge', $endorsement->person_in_charge ?? '');
                                    $selectedUserName =
                                        $users->firstWhere('id', $selectedUserId)->name ?? 'Select a Person in Charge';
                                @endphp

                                <input type="text" id="selected-content-writer-name" value="{{ $selectedUserName }}"
                                    class="w-full border px-3 py-2 border-gray-200 rounded-lg cursor-pointer" readonly
                                    onclick="openContentWriterModal()">
                    ...
                    <div class="col-span-2 h-fit w-full">
                        ...
                        <!-- CKEditor Textarea -->
                        <textarea name="timeline" id="timeline" class="w-full border-gray-200 rounded-lg">{{ old('timeline', $endorsement->timeline) }}</textarea>
                        @error('timeline')
                            <p class="text-red-600 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-2 h-fit w-full">
                        ...
                        <!-- CKEditor Textarea -->
                        <textarea name="project_scope" id="project-scope" class="w-full border-gray-200 rounded-lg">{{ old('project_scope', $endorsement->project_scope) }}</textarea>
                        @error('project_scope')
                            <p class="text-red-600 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-2 h-fit w-full">
                        <p class="text-sm text-gray-600">Deliverables</p>
                        <!-- CKEditor Textarea -->
                        <textarea name="deliverables" id="deliverables" class="w-full border-gray-200 rounded-lg">{{ old('deliverables', $endorsement->deliverables) }}</textarea>
                        @error('deliverables')
                            <p class="text-red-600 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
