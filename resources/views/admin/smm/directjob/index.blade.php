<head>
    <title>{{ env('APP_NAME') }} | SMM | Direct Job Order</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>

@php
    //roles
    $required_roles = ['top_management', 'operations_supervisor'];
    $role = Auth::user()->roles->position;
    $route = null;
    if (!in_array($role, $required_roles)) {
        if ($role == 'top_management') {
            $route = 'admin.smm.topmanager.directjob';
        } elseif ($role == 'operations_supervisor') {
            $route = 'admin.smm.supervisor.directjob';
        }
    }
    ...
    $createRoute = '';
    if ($role == 'top_management') {
        $createRoute = route('admin.smm.topmanager.directjob.create');
    } elseif ($role == 'operations_supervisor') {
        $createRoute = route('admin.smm.supervisor.directjob.create');
    }

    $editRoute = '';
    if ($role == 'top_management') {
        $editRoute = 'admin.smm.topmanager.directjob.edit';
    } elseif ($role == 'operations_supervisor') {
        $editRoute = 'admin.smm.supervisor.directjob.edit';
    }

    $showRoute = '';
    if ($role == 'top_management') {
        $showRoute = 'admin.smm.topmanager.directjob.show';
    } elseif ($role == 'operations_supervisor') {
        $showRoute = 'admin.smm.supervisor.directjob.show';
    }

@endphp

<x-main-layout breadcumb="SMM" page="Direct Job Order">
                ...
                @forelse ($job_drafts as $job_draft)
                    ...
                    : ($job_draft->status == 'Revision'
                        ...
                        : 'bg-[#fa6e117e]') }} ">
                                        ...
                                        <br />
                                        @if ($job_draft->date_target < now()) <span
                                            class="text-sm font-bold text-red-500">{{ $job_draft->date_target }} LATE</span>
                                        @else
                                ...
                                <td class="w-[20%] px-4 py-3 text-center">
                                    <a href="{{ route($editRoute, $job_draft->id) }}">
                                        <button
                                            class="px-2 py-1 mb-2 lg:mb-0 lg:px-4 lg:py-2 text-sm text-white {{ $job_draft->status === 'pending' || $job_draft->status === 'Waiting for Content Writer Approval' || $job_draft->status === 'Waiting for Graphic Designer Approval' ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-400 cursor-not-allowed' }} rounded "
                                            {{ $job_draft->status === 'pending' || $job_draft->status === 'Waiting for Content Writer Approval' || $job_draft->status === 'Waiting for Graphic Designer Approval' ? '' : 'disabled' }}>
                                            Edit
                                        </button>
                                    </a>
                                    <a href="{{ route($showRoute, $job_draft->id) }}">
                                        <button
