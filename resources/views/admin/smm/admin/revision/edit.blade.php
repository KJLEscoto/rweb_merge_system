...
<x-main-layout breadcumb="SMM / My Revisions" page="Operation Revision">
    <div class="px-10 pt-10">
        <div class="w-full px-6 py-10 mx-auto rounded-lg bg-white custom-shadow">
            <div>
                <a href="{{ url('/admin/smm/operation/revision') }}">
                    <div class="w-fit px-4 py-1 bg-gray-400 rounded-md text-white custom-shadow custom-hover-shadow">
                        Back
                    </div>
                </a>
            </div>
            <form action="{{ url('/admin/smm/operation/revision/update/' . $job_draft->id) }}" method="POST">
                @csrf
                @method('PUT')
