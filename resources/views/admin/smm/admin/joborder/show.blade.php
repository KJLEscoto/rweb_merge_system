<head>
    <title>{{ env('APP_NAME') }} | SMM | Job Order Show</title>
...
<x-main-layout breadcumb="SMM / Outgoing Requests" page="Job Order Show">
    <div class="px-10 pt-10">
        <div id="container-pdf">
            <div class="bg-[#fa7011] text-white rounded-md px-3 py-1 w-fit mb-4">
                <a href="{{ url('/admin/smm/joborder') }}">Back</a>
            </div>
            <div class="header">
                        ...
                        <td class="signature">
                            <strong>Assigned Personnel Signature:
                                @if ($job_draft->type == 'content_writer')
                                    ...
                                    @else
                                        <img src="{{ asset($job_draft->signature_worker) }}"
                                            alt="Content Writer Signature">
                                    @endif
                                @else
                                    {{ $job_draft->graphicDesigner->name }}
                                    @if ($job_draft->status === 'Waiting for Content Writer Approval')
