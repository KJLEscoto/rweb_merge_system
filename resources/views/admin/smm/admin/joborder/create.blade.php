<head>
    <title>{{ env('APP_NAME') }} | SMM | Job Order Create</title>
...
<x-main-layout breadcumb="SMM / Outgoing Requests" page="Job Order Create">
                    ...
                    <div class="col-span-4 grid grid-cols-2 gap-4 mt-4">
                        ...
                        <div class="w-full">
                            ...
                            <select name="content_writer_id" class="w-full border-gray-200 rounded-lg text-sm ">
                                <option value="" disabled {{ old('content_writer_id') ? '' : 'selected' }}>Select
                                    A Content Writer</option>
                                @foreach ($content_writers as $content_writer)
                                    <option value="{{ $content_writer->id }}"
                                        {{ old('content_writer_id') == $content_writer->id ? 'selected' : '' }}
                                        class="text-black text-sm">{{ $content_writer->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="w-full">
                            ...
                            <select name="graphic_designer_id" class="w-full border-gray-200 rounded-lg text-sm ">
                                <option value="" disabled {{ old('graphic_designer_id') ? '' : 'selected' }}>
                                    Select A Graphic Designer</option>
                                @foreach ($graphic_designers as $graphic_designer)
                                    <option value="{{ $graphic_designer->id }}"
                                        {{ old('graphic_designer_id') == $graphic_designer->id ? 'selected' : '' }}>
                                        {{ $graphic_designer->name }}</option>
                                @endforeach
                            </select>
