...
<head>
    <title>{{ env('APP_NAME') }} | SMM | My Task</title>

    <script src="https://cdn.tailwindcss.com"></script>
</head>

<x-main-layout breadcumb="SMM" page="My Task">
                        ...
                        <td class="px-6 py-3">
                            @if (
                                ...
                                @if ($job_draft->status == 'pending')
                                    <a href="{{ url('admin/smm/operation/task/create/' . $job_draft->id) }}">
                                        <button
                                ...
                                @elseif (
                                    $job_draft->status == 'Waiting for Content Writer Approval' ||
                                        $job_draft->status == 'Waiting for Graphic Designer Approval')
                                    <form action="{{ url('admin/smm/operation/task/accept/' . $job_draft->id) }}"
                                ...
                                @endif
                                <a href="{{ url('admin/smm/operation/task/show/' . $job_draft->id) }}">
                                    <button
                            ...
                            @elseif ($job_draft->status == 'Submitted to Assistant Supervisor')
                                <a href="{{ url('admin/smm/operation/task/edit/' . $job_draft->id) }}">
                                    <button
    ...
    function filterByStatus(status) {
        ...
        for (let row of rows) {
            ...
            if (status === 'all') {
                ...
                row.style.display = (rowStatus === 'pending' || rowStatus === 'waiting for content writer approval' ||
                    rowStatus === 'waiting for graphic designer approval') ? "" : "none";
            } else {
