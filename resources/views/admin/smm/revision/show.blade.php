<head>
    <title>{{ env('APP_NAME') }} | SMM | Show Revision</title>
...
<x-main-layout breadcumb="SMM / Revision" page="Show Revision">
        ...
        <div class="w-full px-6 py-10 mx-auto rounded-lg custom-shadow bg-white">
            ...
            <div class="mt-10 grid grid-cols-3 gap-8">
                @foreach ($revisions as $revision)
                    {{-- {{$revision}} --}}
                    <div
                        class="rounded-lg shadow-lg {{ $revision->status == 'complete' ? 'bg-green-500' : 'bg-gray-400' }} col-span-1 text-white p-10 mb-10">
                        <div class="mb-10">
                            <p>Revision By:</p>
                        ...
                        <div class="flex items-center justify-center cursor-pointer">
                            <div class="bg-[#fa7011] w-fit px-2 py-1 rounded-md"
                                onclick="showDetails('{{ $revision->declinedBy->name }}', '{{ $revision->revision_date }}', `{!! $revision->summary !!}`, `{!! $revision->last_draft !!}`)">
                                Show Details
                            </div>
                        </div>
                    </div>
