<head>
    <title>{{ env('APP_NAME') }} | SMM | Decline Form</title>
...
<x-main-layout breadcumb="SMM / Approval" page="Decline Form">
        ...
        <div class="">
            <div class="">
                <div class="h-auto gap-8 m-10 p-10 relative bg-white"
                    ...
                    <div class="mt-10">
                        <form action="{{ url('/admin/smm/supervisor/approve/decline/' . $job_draft->id) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            <!-- CKEditor 5 for Decline Reason -->
                            <label for="summary" class="block font-semibold">Decline Reason:</label>
                            <textarea class="w-full border p-2 rounded-md" name="summary"
                                id="summaryEditor">{{ old('summary') }}</textarea>
