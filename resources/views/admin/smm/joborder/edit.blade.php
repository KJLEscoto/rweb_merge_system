<head>
    <title>{{ env('APP_NAME') }} | SMM | Edit Direct Job Order</title>
...
<x-main-layout breadcumb="SMM / Direct Job Order" page="Edit Direct Job Order">

    <div class="w-full px-6 py-10 mx-auto rounded-lg custom-shadow bg-white">
        <div>
            <a href="{{ route('admin.smm.topmanager.directjob') }}">
                <div class="w-fit px-4 py-1 bg-gray-400 rounded-md text-white custom-shadow custom-hover-shadow">
                    Back
                </div>
            </a>
        </div>
        <form action="{{ route('admin.smm.topmanager.directjob.update', $job_draft->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-4 space-y-4">
                <div class="col-span-4 grid grid-cols-2 gap-4 mt-4">
                    ...
                    <div class="col-span-2 h-fit w-full">
                        ...
                        <textarea name="description" id="editor"
                            class="w-full border-gray-200 rounded-lg">{{ old('description', $job_draft->jobOrder->description) }}</textarea>
                ...
                <button type="submit"
                    class="col-span-1 text-center py-4 w-full bg-[#fa7011] mt-10 rounded-lg text-white font-bold">
                    Submit
                </button>
            </div>
        </form>
