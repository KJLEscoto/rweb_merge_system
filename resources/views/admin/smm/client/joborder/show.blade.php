<head>
    <title>{{ env('APP_NAME') }} | SMM | Show Approval</title>
...
<x-main-layout breadcumb="SMM / List Job Order" page="Show Approval">
    <div class="px-10 pt-10">
        <div class="h-auto gap-8 m-4 lg:m-10 p-4 lg:p-10 relative bg-white"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
                ...
                <div class="lg:col-span-4">
                    @php
                        $isDisabled = $job_draft->status != 'Submitted to Client';
                        $isSigned = !empty($job_draft->signature_client);
                    @endphp

                    <div class="mt-6 bg-white p-4 rounded-md shadow-md w-full">
                        ...
                        <form action="{{ url('/smm/client/update/' . $job_draft->id) }}" method="POST"
                            ...
                            <div id="savedPadSection" class="{{ Auth::user()->signature ? '' : 'hidden' }}">
                                @if (Auth::user()->signature)
                                    <img id="new-signature-pad-main" class="w-[300px] lg:w-[400px]"
                                        style="height:200px;" src="{{ asset(Auth::user()->signature) }}"
    ...
    <!-- Decline Modal -->
    <div id="declineModal"
        class="fixed inset-0 flex items-center justify-center hidden bg-gray-500 bg-opacity-50 z-50">
        <div class="bg-white p-6 rounded-md w-[50%]">
            <h2 class="text-xl font-bold mb-4">Decline Job Order</h2>
            <form action="{{ url('/smm/client/decline/' . $job_draft->id) }}" method="POST" id="declineForm">
                @csrf
                <div class="mb-4">
