<head>
    <title>{{ env('APP_NAME') }} | SMM | Show Job Order</title>
...
<x-main-layout breadcumb="SMM" page="Show Job Order">
    ...
    <div id="container-pdf">
        <div class="bg-[#fa7011] text-white rounded-md px-3 py-1 w-fit mb-4">
            <a
                href="{{ route('admin.web.direct-job-order.showProjectChannels', $web_project_channel->web_job_order_id) }}">Back</a>
        </div>
        <div class="header">
        ...
        <div class="section">
            ...
            <table>
                <tr>
                    <td><strong>Client Name:</strong><br>{{ $web_project_channel->web_project->client->name }}</td>
                    <td><strong>Client Address:</strong><br>{{ $web_project_channel->web_project->client->address }}
                    </td>
                </tr>
            </table>
            ...
            <table>
                <tr>
                    <td><strong>Issued by:</strong><br>{{ $web_project_channel->web_project->issuer->name }}</td>
                    <td>
            ...
            <table>
                <tr>
                    ...
                    <td><strong>Time Required:</strong><br>
                        @if ($web_project_channel->date_started && $web_project_channel->date_completed)
                            {{ \Carbon\Carbon::parse($web_project_channel->date_started)->diffInDays(\Carbon\Carbon::parse($web_project_channel->date_completed)) }}
                            days
                        @else
                            N/A
                        @endif
