...
<x-main-layout breadcumb="SMM" page="Edit Revision">
    <div class="px-10 pt-10">
        <div class="w-full px-6 py-10 mx-auto rounded-lg custom-shadow bg-white">
            <div>
                <a href="{{ route('admin.web.revision') }}">
                    <div class="w-fit px-4 py-1 bg-gray-400 rounded-md text-white custom-shadow custom-hover-shadow">
                        Back
                    </div>
                </a>
            </div>
            <form action="{{ route('admin.web.revision.edit', $web_revision->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-span-4 grid grid-cols-2 gap-4 mt-4">
                        <div class="col-span-4 grid grid-cols-5 space-y-2 lg:space-y-0">
                            <div class="col-span-5 lg:col-span-1 w-full">
                                ...
                                <p class="text-xl">{{ $web_revision->web_project_channel->web_project->title }}</p>
                            </div>
                            <div class="col-span-5 lg:col-span-1 w-full">
                                <p class="text-sm text-gray-600 border-[#fa7011] border-b-2 w-fit">Client</p>
                                <p class="text-xl">{{ $web_revision->web_project_channel->web_project->client->name }}
                                </p>
                            </div>
                            <div class="col-span-5 lg:col-span-1 w-full">
                                <p class="text-sm text-gray-600 border-[#fa7011] border-b-2 w-fit">Date Started</p>
                                <p class="text-xl">
                                    {{ \Carbon\Carbon::parse($web_revision->web_project_channel->date_started)->format('Y-m-d') }}
                                </p>
                            </div>
                            <div class="col-span-5 lg:col-span-1 w-full">
                                <p class="text-sm text-gray-600 border-[#fa7011] border-b-2 w-fit">Date Target</p>
                                <p class="text-xl">
                                    {{ \Carbon\Carbon::parse($web_revision->web_project_channel->date_targeted)->format('Y-m-d') }}
                                </p>
                            </div>
                            <div class="col-span-5 lg:col-span-1 w-full">
                                <p class="text-sm text-gray-600 border-[#fa7011] border-b-2 w-fit">Revision By</p>
                                <p class="text-xl">{{ $web_revision->declined_by->name }}</p>
                            </div>
                            ...
                            <div class="col-span-5 w-full mt-4">
                                ...
                                <div
                                    class="text-sm text-gray-600 w-full max-h-[500px] overflow-y-auto bg-white border border-gray-300 p-2 rounded">
                                    {!! $web_revision->web_project_channel->web_project->instructions !!}
                                </div>
                            </div>
                        </div>
