...
<x-main-layout breadcumb="DTR / Interns / Intern Details" page="View DTR">
    <div class="flex flex-col gap-5">
        {{-- <section
                ...
                <div class="flex items-center justify-end lg:col-span-1 gap-3">
                    <form action="{{ route('admin.dtr.interns.dtr.post', ['id' => $user->id]) }}" method="POST"
                        class="inline">
                        @csrf
                        @method('POST')
        ...
        <section
            class="w-full flex items-center justify-between gap-5 bg-white p-3 border border-gray-200 shadow-lg sticky top-[125px] z-30 rounded">
            ...
            <div class="flex items-center justify-end gap-3">
                <form action="{{ route('admin.dtr.interns.dtr.post', $user->id) }}" method="POST" class="m-auto">
                    @csrf
                    @method('POST')
                    <input type="month" name="searchDate" id="searchDate"
                        class="px-5 py-2 rounded cursor-pointer border border-gray-200 text-sm"
                        value="{{ \Carbon\Carbon::parse($pagination['currentMonth']['name'])->format('Y-m') }}"
                        onchange="this.form.submit()">
                </form>
                <button type="button" id="download-pdf-btn"
                    class="bg-[#f56d11] hover:scale-105 transition text-white px-3 py-2 text-sm rounded font-semibold shadow-md w-fit flex items-center justify-center gap-2">
                    <span class="material-symbols--download-rounded w-4 h-4"></span>
                    <p class="md:block hidden">Download PDF</p>
                </button>
            </div>
        </section>
                    ...
                    <div class="w-full h-auto">
                        <div
                            ...
                            <section class="my-7 text-center">
                                <p class="text-[#F57D11] font-semibold sm:text-base text-sm">OJT Daily Time Record
                                </p>
...
document.addEventListener("DOMContentLoaded", function() {
    document.querySelectorAll("[name='download-pdf-btn']").forEach(button => {
        debugger;
        console.log(button.getAttribute("name")); // Log button name (fixing getAttributes issue)
        ...
        button.addEventListener("click", function() {
            fetch("{{ route('admin.dtr.download.pdf') }}", {
                ...
                headers: {
                    ...
                    "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute("content") // Securely get CSRF token
                },
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (button) {
                console.log("Button found:", button); // Debugging
                button.addEventListener("click", function() {
                    alert(1);
                    fetch("{{ route('admin.dtr.download.pdf') }}", {
                            method: "POST",
                            headers: {
                                ...
                                "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]')
                                    .getAttribute("content") // Securely get CSRF token
                            },
                            credentials: "include", // Ensure cookies are sent with the request
                            body: JSON.stringify({
                                records: @json($records),
