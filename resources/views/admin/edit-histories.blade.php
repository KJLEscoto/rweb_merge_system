        ...
        <form id="historyForm" action="{{ route('admin.dtr.history.edit.post', ['id' => $history->id]) }}" method="POST" class="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            ...
            <div class="user-history-card p-4 border rounded-lg shadow-md bg-white relative">
                <div class="flex items-center gap-4">
                    <img src="{{ \App\Models\File::where('id', \App\Models\Profile::where('id', $history->user->profile_id)->first()->file_id)->first()->path . '?t=' . time() ?? 'https://via.placeholder.com/50' }}" class="w-12 h-12 rounded-full" alt="{{ $history->user->firstname }}">
                    <div>
